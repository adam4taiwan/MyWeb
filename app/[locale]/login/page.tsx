...
export default function LoginPage() {
  ...
  const API_BASE_URL = process.env.NEXT_PUBLIC_API_URL
    ...
    : (typeof window !== 'undefined' && window.location.hostname === 'localhost')
      ? 'http://localhost:5013/api/auth'
      : 'https://ecanapi.fly.dev/api/auth';
      ...
      <div className="relative z-10 w-full max-w-md p-8 space-y-6 bg-white rounded-2xl shadow-2xl">
        ...
        {error && (
          <div className={`p-3 border rounded-lg text-sm text-center ${
            error.includes(t('registerSuccess').slice(0, 4))
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          >
            {isLoading ? t('processing') : (isRegister ? t('btnRegister') : t('btnLogin'))}
          </button>
        </form>
