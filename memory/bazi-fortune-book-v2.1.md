---
name: 八字命書版本說明（v2.1-v2.3）
description: 八字紫微命書各版本改版細節，含喜忌表格、大運宮位論斷、紫微章節標題、KbRemoveOtherPalaceLines bug 修正
type: project
---
...
**目的：** 保留大運命書的購買誘因，不在八字命書內給出完整大運分析。

**舊格式：**
```
【11-20歲（童年學藝期）癸卯】運勢 65 分 — 中吉運
  建議文字...
```

**新格式：**
```
一生大運行運一覽（詳細吉凶論斷，請購買【大運命書】）：
  11-20 歲：癸卯
  21-30 歲：甲寅  ← 目前行運（今年 XX 歲）
  31-40 歲：乙丑
  ...
```
...
**涉及函式：** `LfBuildReport()` Ch.10 foreach 迴圈，移除 `LfAgeBandAdvice()`
...
**效果：** DOCX 匯出的 `LfBuildYudongziDocxBytes()` 會自動在 `【第X章：...】` 格式的行前加換頁（條件：bookTitle != "玉 洞 子 命 書" 且 chN >= 2）

---

### 3. KbRemoveOtherPalaceLines 空白字元 bug 修正

**問題：** KB 內容中宮位名稱帶全形空格（如 `命　宮`），導致 `Contains("命宮")` 判斷失效，`命　宮：在(未)--有天相主星` 這行被誤留在夫妻宮 section。

**修正：** 在 `mentionsOther` / `mentionsTarget` 判斷前，先用 Regex 正規化去除空白：
```csharp
